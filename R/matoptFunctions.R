...
glottocode_taxonomy <- function(path_to_file){
  ...
  library(readr)
    ...
    # now we want to normalize the paths — each path must be of the same length, with roots in front and
    # leafs in back
...
## the matrix will now be trimmed iteratively:

# collect the absolute coding proportion of each langauge
abs_prop_non_NA<-(rowSums(full_matrix)/ncol(full_matrix))

# not each entry in a row/column is equivalent in its value:
# if data for a language with good coverage is coded for a variable, it should weigh more than a language that hardly has any information coded; the equivalent is true for rows --> compute weighted row and column means
...
prop_NA <- data.frame(iterations = "0 (full)",
...
mean_type = "log_odds"
taxonomy = TRUE
max_steps = nrow(full_matrix)+ncol(full_matrix)

set.seed(100)

tic()
# as it is, the trimming algorithm stops after 3279 iterations after ~35 minutes!
for (iterations in 1:max_steps){
  ...
  } else if (mean_type == "log_odds") { # log odds mean
  ...
  cat("Identifying worst languages and variables:", rownames(worstlg), "and", worstvar ,"\n")
  ...
  if(worstlg$mean_score < min(c_weights_updated)){
    cat("Remove the worst language:",rownames(worstlg),"\n")
    ...
    register <- register[register$glottocode%in%removed_lgs==F,] # update register by pruning away worst lg
  ...
  if(worstlg$mean_score > min(c_weights_updated)){
    cat("Remove the worst variable:", worstvar,"\n")
  ...
  cat("Ensuring language variablity.\n")
  ...
  if(min(rowSums(updated_matrix))==0){
      ...
      register <- register[register$glottocode%in%uninformative_languages==F,] # update register by pruning away worst lg
...
pruning_score <- function(documentation, exponent_prop_coded_data, exponent_available_data_points){
  quality <- as.numeric(prop_NA$prop_coded_data)^exponent_prop_coded_data*as.numeric(prop_NA$available_data_points)^exponent_available_data_points
  plot(quality,xlab="iteration",ylab="quality score")
  optimum <- which(quality==max(na.omit(quality)))
  return(optimum)
}

optimum <- pruning_score(prop_NA,exponent_prop_coded_data=1, exponent_available_data_points=1)
...
pruned_matrix <- densify_prune(original_data, prop_NA, optimum)
pruned_matrix <- densify_prune(original_data, prop_NA, pruning_score(prop_NA,exponent_prop_coded_data=1, exponent_available_data_points=1))
