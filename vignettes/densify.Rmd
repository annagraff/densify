...
package: the language-variable matrix `WALS` obtained from WALS, the
...
variables (in linguistic typological databases, these are also often
referred to as "features", "characters" or "parameters").
<!--again, check whether we shouldn't globally use "feature"--> The data
...
meta-data (such as taxon names) or variables. Variable names must
...
<!-- I see only data/glottolog_languoids.rda Please check, or make clear that the df can be loaded with data()-->
...
`r ncol(WALS[,-which(names(WALS)=="Glottocode")])` linguistic variables,
...
In this data matrix each variable is coded for a distinct set of
...
\@ref(fig:patchy-coding-for-languages-and-variables)).

<!--"unpruned" is not so common I think. Perhaps better "raw", like in the directory?-->
...
`r round(sum(!is.na(data))/(ncol(data)*nrow(data)),2)`%.

```{r overall coding density input data frame, echo = TRUE}
sum(!is.na(data))/(ncol(data)*nrow(data))
```

<!-- simpler and easier: mean(!is.na(data)) or is the more complex code on purpose? -->
...
`densify()` accomplishes this: It takes the original data matrix,
<!--pls be consistent: either always frame or always matrix; I'd go for matrix throughout as data frame is an R-specific tech term-->
iteratively removes rows and columns according to specifiable parameters
<!--pls make sure you are consistent wrt "parameters" vs "settings"-->
...
containing zeroes for NAs and ones elsewhere. Using this encoding
matrix, it computes a **importance score for each row and each column**
according to user-specifiable settings, identifies the row and/or column
...
Thus, to determine the importance score of a column (variable),
...
removal of rows to slowly deplete a variable state, such that a variable
...
`densify()` assesses whether any variables have become uninformative
according to a user-defined threshold denoting how many observations
must be present in the second-largest variable state, and removes any
...
-   `cols`: A specification of which variable columns to densify. The
...
-   `scoring`: This parameter specifies how the measure denoting row
    importance is computed. It can be one of the following three
    mean-types: `"arithmetic"`, `"geometric"`, `"log_odds"`. See below
    for more details. The default setting is `"log_odds"`.

-   `min_variability`: An integer specifying how many observations/taxa
    the second-most-frequent variable state of any variable must contain
    for the variable to be maintained in the matrix. The default set to
    `1`, guaranteeing that a variable must display at least minimal
...
-   `limits`: A named list specifying the limit conditions for the
    ...
    specifying the minimal proportion of variable columns that have to
...
-   `scoring_weights`: A named list specifying weighting factors applied
    ...
    the pruning process. The default setting is
...
<!--Pls double check. I tried to make this more transparent but I hope I didn't misunderstand!-->
...
setting `scoring_weights`, where weights can be specified for `coding`
and `taxonomy`. These are factors $\in [0,1]$, defaulting to `1`.
<!-- \in [0,1] suggests a continuous value, so not a factor... Perhaps use 'multiplier' or 'weight'?-->
...
computation, and the `taxonomy` weight specifies the factor by which the
taxonomic diversity score is multiplied by. If set to different values,
...
for each sub-matrix in the `densify\_result` output produced by
...
In summary, the parameters `scoring` and `scoring_weights` modulate the
pruning process and can (and should) be adjusted to meet the needs of
the user. It may be necessary to experiment with different settings to
find a good balance of parameters.

#### Parameter `min_variability`

While it is conceivable that one might want to include variables in a
dataset displaying no variability whatsoever among the coded
taxa/observations (`min_variability = NA`), such variables are
uninteresting for most statistical purposes.

The parameter `min_variability` specifies how many taxa the
second-most-frequent variable state of any variable must contain for the
variable to be maintained in the matrix. The default is `1`, i.e. there
must be at least one language that has a value different from all
others. This will remove variables with only one state -- whether they
are present in the input data matrix or whether they result from
...
variable contains at least two values and that the second-most frequent
...
In this data frame, variables `A` and `B` each have two states, `absent`
and `present`, but variable `B` exhibits more variation than variable
`A`: The rarer state `present` counts two observations, whereas it
counts only one observation in variable `A`. Variables `C` and `D` each
have three states, `type1`, `type2` and `type3`, but variable `D`
exhibits more variation than variable `C`: The second-most-frequent
state `type2` counts two observations instead of just one (variable
`C`).

Different settings `min_variability` on this data have the following
effect:

-   `min_variability = NA`: All variables are retained, including
    variable `F`, which displays no variability among observations.

-   `min_variability = 1`: Variables `A`, `B`, `C`, `D` and `E` are
    retained, because each of them contains at least one observation in
    the second-most-frequent state. Variable `F` is removed, because it
    only includes one state.

-   `min_variability = 2`: Variables `B` and `D` are retained, because
    both of them contain at least two observations in the
    second-most-frequent state. Variables `A`, `C`, `E` and `F` are
...
discussed above: `scoring` and `scoring_weights`. Their outputs as well
...
-   `changes`: a tibble listing all pruned taxa and/or variables in the
...
mean(!is.na(as.data.frame(dr_lom_taxtrue_11$data[[2440]]))) == dr_lom_taxtrue_11$coding_density[2440]
...
`score` (as a standard R expression) and biases the sub-matrix selection in the  way the trade-offs are meant to play out. The default formula used is
`score = n_data_points * coding_density`, which maximizes the product of
...
with respect to the score they specify (returning a vector indicating
the rank of each sub-matrix), while `prune()` identifies and retrieves
the sub-matrix for which the score is maximized (selecting the earlier
...
optimum.
<!--Perhaps remove the max score print as it has no immediately intuition. Step nr. and df dims are super useful but the score value itself perhaps not as much -->
...
times on the same densification output using scores to fine-tune the
settings according to their needs. We recommend the choice of the score
...
`score = n_data_points * coding_density * row_coding_density_min`). If
this concern is strong, the importance of `row_coding_density_min` can
be increased by an exponent (e.g.,
`score = n_data_points * coding_density * row_coding_density_min^2`).
...
`score = n_data_points * coding_density^2 * row_coding_density_min * col_coding_density_min`).
...
`score = n_data_points^0.5 * coding_density * taxonomic_index`).
...
scores on the same iteration log (obtained from an implementation
of `densify()` using the settings
`scoring = "arithmetic", consider_taxonomic_diversity = TRUE`.) The quality score per iteration and the optimum sub-matrix vary
drammatically depending on the score function. Accordingly, so do the
...
`score = n_data_points * coding_density * taxonomic_index` encompasses
`r nrow(pruned_1)` languages in
`r flat_taxonomy_matrix %>% filter(id %in% pruned_1$Glottocode) %>% select(level1) %>% unique() %>% nrow()`
families coded for
`r ncol(pruned_1[,-which(names(pruned_1)=="Glottocode")])` variables at
...
Both language and variable coding densities have become
substantially higher (see Figure \@ref(fig:prune-plots-1)) as compared
to the input matrix (see Figure
\@ref(fig:patchy-coding-for-languages-and-variables)), and how the
inclusion of taxonomic diversity in both `densify()` and the `score`
...
`score = n_data_points * coding_density * row_coding_density_min * col_coding_density_min * taxonomic_index`
...
families coded for
`r ncol(pruned_2[,-which(names(pruned_2)=="Glottocode")])` variables at
an overall coding density of
...
`col_coding_density_min` in the `score` argument thus results in a much
...
`densify()` and the `score` parameter in `rank_results()`, `prune()` and
`visualize()`, we set 10 different `score` definitions in `prune()` for
...
the number of variables maintained. Each of these values are compared to
