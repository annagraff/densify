...
```{r densify-result, eval = TRUE, echo = FALSE}
...
Note how both language and variable coding densities have become very high (see Figure \@ref(fig:prune-plots-2)), and how the inclusion of taxonomic diversity in both `densify()` and the `score` argument of `prune()` result in the inclusion of a number of languages that are not densely coded, but strongly contribute to taxonomic diversity and are thus kept.
...
ggplot(data.frame(density=apply(data_1, 1, function(x) (length(na.omit(x))))/ncol(data_1)), aes(x=density))+
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  xlim(c(-0.1,1.1))+
  labs(title="Pruned WALS, arithmetic, taxonomy used", 
...
Additionally including `row_coding_density_min` and `col_coding_density_min` in the `score` argument thus result in a much denser matrix with well-coded rows and columns.
...
for (i in 1:length(iteration_log_files)){
  ...
  for (k in 1:length(scores)){
    ...
    logfile <- rbind(logfile,c(mn, min_variability, taxonomy, seed, twf, cwf, docname,
                             score, cprop, nlg, nfam, nvar, round(cprop/full_coding_proprotion,3), round(nlg/full_n_lg,3), round(nfam/full_n_fam,3), round(nvar/full_n_var,3)))
  }
}
